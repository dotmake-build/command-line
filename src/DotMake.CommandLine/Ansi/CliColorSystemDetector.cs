...
internal static class CliColorSystemDetector
{
    // Adapted from https://github.com/willmcgugan/rich/blob/f0c29052c22d1e49579956a9207324d9072beed7/rich/console.py#L391
    public static CliColorSystem Detect(bool supportsAnsi)
    {
        // No colors?
        if (Environment.GetEnvironmentVariables().Contains("NO_COLOR"))
        {
            return CliColorSystem.NoColors;
        }
        ...
        if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
        {
            if (!supportsAnsi)
            {
                ...
                return CliColorSystem.EightBit;
            }
            ...
            if (GetWindowsVersionInformation(out var major, out var build))
            {
                ...
                {
                    return CliColorSystem.TrueColor;
                }
                else if (major > 10)
                {
                    return CliColorSystem.TrueColor;
