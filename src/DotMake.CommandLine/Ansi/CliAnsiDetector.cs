...
internal static class CliAnsiDetector
{
    ...
    public static (bool Ansi, bool Legacy) Detect(TextWriter buffer, CliAnsiSupport ansiSupport)
    {
        return Detect(
            buffer,
            ansiSupport,
    ...
    internal static (bool Ansi, bool Legacy) Detect(
        TextWriter buffer,
        CliAnsiSupport ansiSupport,
        bool isOutputRedirected,
        bool isErrorRedirected)
    {
        var supportsAnsi = ansiSupport == CliAnsiSupport.Yes;
        var legacyConsole = false;

        if (ansiSupport == CliAnsiSupport.Detect)
        {
            ...
            (supportsAnsi, legacyConsole) = CliAnsiDetector.Detect(buffer.IsStandardError(), true);
        ...
        if (buffer.IsStandardOut() || buffer.IsStandardError())
        {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                ...
                (_, legacyConsole) = CliAnsiDetector.Detect(buffer.IsStandardError(), false);
            }
        }
