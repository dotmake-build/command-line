...
{
    public class CliOptionInput : InputBase, IEquatable<CliOptionInput>
    {
        ...
        public CliOptionInput(ISymbol symbol, SyntaxNode syntaxNode, AttributeData attributeData, SemanticModel semanticModel, CliCommandInput parent)
            : base(symbol, syntaxNode, semanticModel)
        {
            ...
            if (AttributeArguments.TryGetValue(nameof(CliOptionAttribute.Order), out var order))
                Order = (int)order;
            ...
            if (AttributeArguments.TryGetValue(nameof(CliOptionAttribute.Group), out var groupValue))
            {
                Group = (groupValue as string)?.Trim();
                Required = false;
            }
        }
        ...
        public bool Equals(CliOptionInput other)
        {
